...
<div class="properties properties-import">
	<div class="clearfix">
	<h2><?php echo __l('Import your properties from') . ' ';?></h2><span class="airbnb-logo"><?php echo __l('AirBnB'); ?></span></div>
	<?php if($steps <= 3):  ?>
		<ul id="stage" class="stage tab-link clearfix">
			<li class="<?php if($steps == 1): ?>highlight<?php endif; ?> <?php if($steps >= 1): ?>active<?php endif; ?>"><?php echo __l('AirBnB Login');?></li>
			<li class="<?php if($steps == 2): ?>highlight<?php endif; ?> <?php if($steps >= 2): ?>active<?php else: ?>inactive<?php endif; ?>"><?php echo __l('Import Properties');?></li>
		</ul>
	<?php endif; ?>
			...
			<div <?php if($steps > 2): ?>class="hide"<?php endif;?>>
							...
							<?php if (!empty($properties)): ?>
								<?php foreach($properties as $property): ?>
									...
									<tr>
										...
										<td class="dl"><?php echo $property['Property']['title']; ?><?php echo $this->Form->input('Property.' . $property['Property']['id'] . '.title', array('type' => 'hidden', 'value' => $property['Property']['title'])); ?></td>
									</tr>
								<?php endforeach; ?>
							<?php else: ?>
								<tr>
				...
				<div class="submit-block clearfix">
					...
					<div class="cancel-block">
						<?php echo $this->Html->link(__l('Cancel'), array('controller' => 'properties', 'action' => 'index', 'type' => 'myproperties', 'status' => 'imported'), array('class' => 'cancel-link', 'title' => __l('Cancel'), 'escape' => false));?>
					</div>
				</div>
			</div>
		<?php endif; ?>
