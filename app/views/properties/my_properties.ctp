...
<table class="list properties-list js-view-count-update {'model':'property','url':'<?php echo $view_count_url; ?>'}">
	...
	<td rowspan="2" class="actions properties-title dl">
		...
		?>
			<th><?php echo $this->Paginator->sort(__l('Per Hour').' ('.Configure::read('site.currency').')','price_per_night');?></th>
	        <th><?php echo $this->Paginator->sort(__l('Per Day').' ('.Configure::read('site.currency').')','price_per_week');?></th>
	        <th><?php echo $this->Paginator->sort(__l('Per Week').' ('.Configure::read('site.currency').')','price_per_month');?></th>
        <?php	
			}else{
		?>
			<th><?php echo $this->Paginator->sort(__l('Per Night').' ('.Configure::read('site.currency').')','price_per_night');?></th>
	        <th><?php echo $this->Paginator->sort(__l('Per Join').' ('.Configure::read('site.currency').')','price_per_week');?></th>
	        <th><?php echo $this->Paginator->sort(__l('Other').' ('.Configure::read('site.currency').')','price_per_month');?></th>
	...
	<tr<?php echo $class;?>>
			...
			if(!empty($property['PropertiesExtraJoinables'])){
				...
				if($property['PropertiesExtraJoinables'][0]['item_price_hour'] != ''){
					$property['Property']['price_per_night'] = $property['PropertiesExtraJoinables'][0]['item_price_hour'];
				}
				if($property['PropertiesExtraJoinables'][0]['item_price_day'] != ''){
					$property['Property']['price_per_week'] = $property['PropertiesExtraJoinables'][0]['item_price_day'];
				}
				if($property['PropertiesExtraJoinables'][0]['item_price_week'] != ''){
					$property['Property']['price_per_month'] = $property['PropertiesExtraJoinables'][0]['item_price_week'];
				}
			}
