...
<?php echo $this->Html->link(__l('List your property'),  array('controller' => 'joinproperties', 'action' => 'add'), array('title' => __l('List your property')));?><?php  else: ?>
	...
	<p class="import-property dr grid_right">
		...
		<ul id="stage" class="stage tab-link clearfix">
			...
			<li class="tb <?php if($steps == 1): ?>highlight<?php endif; ?> <?php if($steps >= 1): ?>active<?php else: ?>inactive<?php endif; ?>"><?php echo __l('General');?></li>
	...
	<?php echo $this->Form->create(null, array('class' => 'normal form-request add-property check-form js-geo-submit {is_required:"true"}', 'enctype' => 'multipart/form-data'));?>
		...
		<?php		if($this->Auth->user('user_type_id') == ConstUserTypes::Admin): ?>
            ...
            <fieldset>
                ...
                <div class="padd-center">
                    <p class="round-5"><?php echo __l('About Your Place'); ?></p>
                    <div class="bed-room-block">
                        <?php echo $this->Form->input('Property.property_type_id', array('label' => __l('Property Type'), 'empty' => __l('Please Select'))); ?>
                    </div>

                    <div class="clearfix">
                        <?php
                            echo $this->Form->input('Property.accommodates', array('type' => 'select', 'label' => __l('Accommodates'), 'empty' => __l('Please Select'), 'options' => $accomadation));
                            echo $this->Form->input('Property.room_type_id',array('type' => 'select', 'id' => 'js-rooms', 'default' => $room_default));
                        ?>
                    </div>
                    <div class="bed-room-block">
                    <?php
                    ...
                    <div class="clearfix">
                        <?php echo $this->Form->input('Property.bed_type_id', array('type' => 'select','default' => $bed_default)); ?>
                    </div>
                      <div class="bed-room-block">
                    <?php
                    ...
                    <?php  echo $this->Form->input('Property.size', array('label' => __l('Property Size'), 'type'=>'hidden', 'value'=>0));
                ?>
                    </div>
                <div class="clearfix">
                    <?php echo $this->Form->input('Property.measurement',array('type'=>'hidden','options'=>$moreMeasureActions,'value'=>0, 'type'=> 'hidden')); ?>
                </div>
                <?php
                    echo $this->Form->input('Property.street_view',array('options' => $moreStreetActions,'div'=>'js-street-container input select', 'type' => 'hidden'));
                    echo $this->Form->input('Property.is_street_view',array('type' => 'hidden'));
                    echo $this->Form->input('Property.is_pets',array('label'=>__l('Pets live on this property'), 'type' => 'hidden'));
                ?>
                </div>
                <div class="padd-bg-bl">
                    <div class="padd-bg-br">
					...
					<fieldset>
						...
						<div class="padd-center">
							...
							<p class="page-information page-information1">Enter a title and description to present your property.</p>
				...
				<fieldset>
    			 ...
    			 <div class="padd-center">
						<h2><?php echo __l('Listing location'); ?></h2>
						<p class="page-information page-information1">Please enter the location of your property.</p>
						 ...
						 <div class="address-left-block grid_14 omega alpha">
							...
							<?php
								...
								echo $this->Form->input('Property.address1', array('id' => 'js-street_id','type' => 'text', 'label' => 'Address'));
                            ...
                            <div class="clearfix address-input-block">
                                        <?php
                                            echo $this->Form->input('Property.address', array('type' => 'hidden','secure' =>false, 'id' => 'PropertyAddressSearch','info'=>'Enter:   Address, City, State, Country <br/><br/>
                    (Note: Address suggestion will be listed when you enter location.)'));
                                        ?>
                            </div>
					...
					<div class="padd-bg-bl">
						...
						<div class="padd-center">
    						<div class="amenities-list">
    							<h2 class="label-type"><?php echo __l('Amenities'); ?></h2>
    							<div class="checkbox-right clearfix" style="height: 250px !important;">
    					...
    					<div class="padd-center">
    						<div class="amenities-list">
    							<h2 class="label-type"><?php echo __l('Holiday Types'); ?></h2>
			...
			<?php if($steps >= 3 ):  ?>
				<div <?php if($steps > 3 ): ?>class="hide"<?php endif;?>>
					<fieldset>
						...
						<div class="padd-center">
							...
							echo $this->Form->input('Property.price_per_night',array('label'=>'Price Per Night ' . Configure::read('site.currency') . ''));
							echo $this->Form->input('Property.price_per_week',array('label'=>'Price Per Week ' . Configure::read('site.currency').''));
							echo $this->Form->input('Property.price_per_month',array('label'=>'Price Per Month ' . Configure::read('site.currency').''));
							// @todo "What goodies I want (Host)" ?>
							<div class="clearfix guest-price-block">
							<?php echo $this->Form->input('Property.additional_guest_price' ,array('label'=>'Additional Guest Price' . ' ' . Configure::read('site.currency') . '')); ?>
							<span class="price-infor"><?php echo 'per night for each additional guest after'; ?></span>
							...
							<span class="info"><?php echo  'If you enable negotiable then Traveler will contact you for negotiation'; ?></span>
							<?php
				...
				<fieldset>
						...
						<div class="padd-center">
							...
							echo $this->Form->input('Property.minimum_nights',array('label' => 'Minimum Nights', 'type'=>'select','options'=>$minNights));
							echo $this->Form->input('Property.maximum_nights',array('label' => 'Maximum Nights', 'type'=>'select','options'=>$maxNights,'empty'=>__l('No Maximum')));
							echo $this->Form->input('Property.house_rules', array('label' => 'House Rules'));
							?>
						<div class="clearfix checkin-checkout-block pr date-time-block">
							<div class="input date-time clearfix">
								<div <?php if($steps == 3 ): ?>class="js-time"<?php endif;?>>
									<?php
										echo $this->Form->input('Property.checkin', array('type' => 'time', 'timeFormat' => 12, 'label' => 'Check in'));
									?>
								</div>
							</div>
							...
							<div class="input date-time end-date-time-block clearfix">
								<div <?php if($steps == 3 ): ?>class="js-time"<?php endif;?>>
									<?php
										echo $this->Form->input('Property.checkout', array('type' => 'time', 'timeFormat' => 12, 'label' => 'Check out'));
									?>
								</div>
							</div>
						</div>
			...
			<?php if($steps >= 4 ):  ?>
				<div <?php if($steps > 4 ): ?>class="hide"<?php endif;?>>
					<fieldset>
						...
						<div class="padd-center">
							<p class="round-5"><?php echo 'Private Details'; ?></p>
								<p class="page-information page-information1"><?php echo 'Private details will be shown after booking request has been confirmed'; ?></p>
								<?php
								echo $this->Form->input('Property.house_manual',array('label' => 'House Manual', 'info' => 'Private: Traveler will get this information after confirmed reservation. For example, Parking information, Internet access details.'));
	                            ...
	                            echo $this->Form->input('Property.location_manual', array('label' => 'Location manual', 'info' => ' Enter complete location details like landmark, complete address, zip code, access details, etc'));
								?>
						</div>
						<div class="padd-bg-bl">
							<div class="padd-bg-br">
						...
						<div class="padd-center">
							...
							<?php
								...
								if (!Configure::read('property.is_enable_edit_property_image')):
									$message .= 'Shared property images cannot be delete once uploaded. So please be sure about property images before complete this step.' . ' ';
								endif;
