...
<?php //echo $this->Html->css('extras.css'); ?>
...
<div class="my-property-inner-block clearfix">
    ...
    <?php // echo $this->element('properties-calendar', array('property_id' => isset($this->request->params['named']['property_id']) ? $this->request->params['named']['property_id'] : '', 'config' => 'sec')); ?>
			...
			<table class="revenues-list list">
				<tr>
					<th rowspan="2"><?php echo __l('Message'); ?></th>
					<th rowspan="2" style="min-width:75px;"><?php echo $this->Paginator->sort(__l('Check in'),'checkin'); ?></th>
<!-- 					<th rowspan="2"><?php echo $this->Paginator->sort(__l('Check out Date'),'checkout'); ?></th>  -->
					<th rowspan="2" style="min-width:75px;"><?php echo $this->Paginator->sort(__l('Property'),'Property.title'); ?></th>
					<th rowspan="2"><?php echo $this->Paginator->sort(__l('Traveler'),'User.username'); ?></th>
					<th rowspan="2" style="min-width:45px;"><?php echo $this->Paginator->sort(__l('Trip Id'),'id'); ?></th>
					<th rowspan="2"><?php echo $this->Paginator->sort(__l('People'),'guests'); ?></th>
					<!-- @todo "Guest details" -->
...
<!--					<th rowspan="2"><?php echo $this->Paginator->sort(__l('Private Note'),'host_private_note'); ?></th>  -->
					<?php if(!empty($this->request->params['named']['status']) && $this->request->params['named']['status'] == 'waiting_for_acceptance'): ?>
							<th rowspan="2"><?php echo __l('Accept');//$this->Html->cText($propertyUser['PropertyUser']['top_code']);?></th>
							<th rowspan="2"><?php echo __l('Reject');  //$this->Html->cText($propertyUser['PropertyUser']['bottum_code']);?></th>
					<?php endif; ?>
			...
			?>
				<tr<?php echo $class;?>>
					<td class="action-block">
						...
						echo $this->Html->link(__l('Activities'), array('controller' => 'messages', 'action' => 'activities',  'order_id' => $propertyUser['PropertyUser']['id']), array('class' => 'view-activities'));
					...
					<td><?php echo $this->Html->link($this->Html->cText($propertyUser['UserProfile']['first_name'].' '.$propertyUser['UserProfile']['last_name'], false), array('controller' => 'users', 'action' => 'view', $propertyUser['User']['username'] ,'admin' => false), array('title'=>$this->Html->cText($propertyUser['User']['username'],false),'escape' => false)); ?>
					</td>
...
<!--   trabajo con accept y cancel  -->
					<?php if(!empty($this->request->params['named']['status']) && $this->request->params['named']['status'] == 'waiting_for_acceptance'): ?>
						<td> <?php echo $this->Html->link(__l('Confirm'), array('controller' => 'property_users', 'action' => 'update_order', $propertyUser['PropertyUser']['id'],  'accept', 'admin' => false, '?r=' . $this->request->url), array('class' => 'confirm js-delete', 'title' => __l('Confirm'))); ?></td>
						<td><?php echo $this->Html->link(__l('Reject'), array('controller' => 'property_users', 'action' => 'update_order', $propertyUser['PropertyUser']['id'],  'reject', 'admin' => false, '?r=' . $this->request->url), array('class' => 'cancel js-delete', 'title' => __l('Reject'))); ?></td>
					<?php endif; ?>
