...
<div class="js-responses">
	<?php echo $this->Form->create('PropertyUser', array('action' => 'process_checkinout', 'class' => 'normal js-ajax-form'));//js-ajax-form?>
		<?php 
			...
			if(!empty($this->request->params['named']['p_action'])):
				$s_label = (($this->params['named']['p_action'] == 'check_out') ? __l('Check out') : __l('Check in'));
				$label = (($this->params['named']['p_action'] == 'check_out') ? __l('Check out Date') : __l('Check in Date'));
				$p_action = $this->params['named']['p_action'];
		...
		<div class="clearfix">
	<?php echo $this->Form->submit($s_label); ?>
    </div>
