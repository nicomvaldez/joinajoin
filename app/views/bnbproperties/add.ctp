...
<div class="properties properties-add form js-responses">
	<?php
	if(!empty($this->request->data['Bnbproperty']['request_id']) && (isset($this->request->params['pass'][0]) && $this->request->params['pass'][0]=='request')){
		echo $this->element('property-index', array('config' => 'sec', 'type' => 'property', 'request_id' => $this->request->data['Bnbproperty']['request_id'], 'request_latitude' => $this->request->data['Bnbproperty']['request_latitude'], 'request_longitude' => $this->request->data['Bnbproperty']['request_longitude']));
	}
	?>
	<?php
if(!empty($this->request->data['Bnbproperty']['request_id']) && (isset($this->request->params['pass'][0]) && $this->request->params['pass'][0]=='request')):?>
<?php echo $this->Html->link(__l('List your property'),  array('controller' => 'properties', 'action' => 'add'), array('title' => __l('List your property')));?><?php  else: ?>
	<div class="clearfix">
	...
	<p class="import-property dr grid_right">
		<?php echo $this->Html->link(__l('Import Properties'), array('controller' => 'properties', 'action' => 'import', 'admin' => false), array('class' => 'import-properties','title' => __l('Import Properties')));?></p></div>
		<?php if($steps <= 8):  ?>
	...
	<?php endif; ?>
	<?php echo $this->Form->create('Bnbproperty', array('class' => 'normal form-request add-property check-form js-geo-submit {is_required:"true"}', 'enctype' => 'multipart/form-data'));?>
     ...
     <div class="padd-center">
					...
					<?php
                        echo $this->Form->input('address', array('label' => __l('Address'), 'id' => 'PropertyAddressSearch','info'=>'Address suggestion will be listed when you enter location.<br/>
(Note: If address entered is not exact/incomplete, you will be prompted to fill the missing address fields.)'));
                    ?>
							</div>
							<?php
								$class = '';
								if (empty($this->request->data['Bnbproperty']['address']) || ( !empty($this->request->data['Bnbproperty']['address1']) && !empty($this->request->data['City']['name']) &&  !empty($this->request->data['Bnbproperty']['country_id']))) {
									$class = 'hide';
								}
							?>
							...
							<?php
								...
								echo $this->Form->input('Bnbproperty.address1', array('id' => 'js-street_id','type' => 'text', 'label' => __l('Address')));
	                            echo $this->Form->input('Bnbproperty.country_id',array('id' => 'js-country_id', 'empty' => __l('Please Select')));
					...
					<div class="submit-block clearfix">
                         <?php
    						if($steps == 1):
    							echo $this->Form->submit(__l('Next'),array('name' => 'data[Bnbproperty][step1]','class' => 'inactive-search','id' => 'js-sub',));
    						endif;
                         ?>
                    </div>
				</div>
			...
			<?php if($steps >= 2 ):  ?>
				<div <?php if($steps > 2): ?>class="hide"<?php endif;?>>
					...
					<fieldset>
						...
						?>
						 ...
						 <div class="clearfix">
							...
							<div class="radio-tabs-rblock grid_left">
						<?php
							echo $this->Form->input('measurement',array('type'=>'radio','legend'=>false,'div'=>'js-radio-style pr','options'=>$moreMeasureActions,'default'=>1));
						?>
						</div>
						</div>
						</div>
					...
					<div class="submit-block clearfix">
					<?php 	if($steps == 2):
								echo $this->Form->submit(__l('Next'),array('name' => 'data[Bnbproperty][step2]'));
							endif; ?></div>
				</div>
			<?php endif; ?>
			<?php if($steps >= 3 ):  ?>
				...
				<fieldset>
					...
					<div class="submit-block clearfix">
                    <?php 	if($steps == 3):
								echo $this->Form->submit(__l('Next'),array('name' => 'data[Bnbproperty][step3]'));
							endif; ?>
                    </div>
				</div>
			<?php endif; ?>
			<?php if($steps >= 4 ):  ?>
				<div <?php if($steps > 4 ): ?>class="hide"<?php endif;?>>
					...
					<div class="submit-block clearfix">
                    <?php 	if($steps == 4):
									echo $this->Form->submit(__l('Next'),array('name' => 'data[Bnbproperty][step4]'));
								endif;   ?>
                    </div>
				</div>
			<?php endif; ?>
							...
							<div class="picture">
								<ol class=" upload-list clearfix">
									<?php	for($i = 0; $i<Configure::read('properties.max_upload_photo'); $i++):  ?>
										<li class="dc">
											<div class="property-img dc pr"> <?php echo $this->Form->uploader('Attachment.'.$i.'.filename', array('id' =>'Attachment.'.$i.'.filename', 'type'=>'file', 'uPreview' => '1', 'uFilecount'=>1, 'uController'=> 'properties', 'uId' => 'PropertyImage'.$i.'',  'uFiletype' => Configure::read('photo.file.allowedExt'))); ?> 
	...
	<div class="padd-bg-tl">
					...
					<div class="submit-block clearfix">
						<?php if($steps == 5):
											echo $this->Form->submit(__l('Finish'),array('name' => 'data[Bnbproperty][step5]'));
										endif; ?>
                    </div>
				</div>
			<?php endif; ?>
