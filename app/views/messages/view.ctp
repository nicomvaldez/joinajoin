	 ...
	 <p class="user-status-info user-status-information">
    	<?php
			if($this->Auth->sessionValid() && ($this->Auth->user('user_type_id') == ConstUserTypes::Admin)):
				...
				if($message['User']['user_type_id'] != ConstUserTypes::Admin):
		...
		?>
			...
			<?php
                ...
                ?>
					<?php if(ConstUserTypes::Admin == $this->Auth->user('user_type_id')):?>
			...
			<div class="message-block-right grid_2 omega alpha clearfix">
				...
				<?php if(ConstUserTypes::Admin != $message['OtherUser']['user_type_id'] && empty($message['Message']['is_sender'])): ?>
					<div class="cancel-block">
					    <li class="home">
                            <span><?php	echo $this->Html->link(__l('Reply') , array('controller' => 'messages', 'action' => 'compose', $message['Message']['hash'],'reply') , null, array('inline' => false));?></span>
                        </li>    
					</div>
				<?php endif;?>
			</div>
              ...
              <div class="mail-body js-corner mail-content-curve-middle">
				...
				<?php if (($message['Message']['is_sender'] == 1) || ($message['Message']['is_sender'] == 2)) : ?>
                   <span class="sender-name"> 
					<?php echo __l('From').': ';  ?>		
						<?php
							if(ConstUserTypes::Admin == $message['OtherUser']['user_type_id']): 
			   ...
			   <div class="js-show-mail-detail-div" style="display:none;">
				<?php
                    if ($message['Message']['is_sender'] == 0) : ?>
                    	<p  class="clearfix" ><span class="show-details-left message-title"><?php echo __l('from').': ';  ?></span> <?php echo $message['OtherUser']['username']; ?> < <?php echo $message['OtherUser']['email']; ?> ></p>
                    <?php
                    else : ?>
                    	<p  class="clearfix"><span class="show-details-left message-title"><?php echo __l('from').': ';  ?></span> <?php echo $message['User']['username']; ?> < <?php echo $message['User']['email']; ?> ></p>
        			<?php
                    endif; ?>
    				<p  class="clearfix"><span class="show-details-left message-title"><?php echo __l('to').': ';  ?></span><?php echo $show_detail_to; ?></p>
                ...
                <?php if(!empty($message['Message']['property_user_id'])):?>
              	<p  class="clearfix"><span class="show-details-left message-title"><?php echo __l('Booking').': ';  ?></span>
				...
				<?php endif;?>
				<p class="subject-info clearfix"><span class="show-details-left message-title"><?php echo __l('Subject').': ';  ?></span>
					<?php
						if($this->Auth->sessionValid() && ($this->Auth->user('user_type_id') == ConstUserTypes::Admin) && !empty($message['MessageContent']['is_system_flagged'])):
