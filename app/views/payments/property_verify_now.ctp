...
<div class="l-curve-top">							
	...
	<h2><?php echo __l(' Pay Verification Fee');?></h2>
	...
	<?php  if(!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='contact'):?>
		<h2><?php echo __l('Booking Details');?></h2>
	...
	<?php elseif(!empty($this->request->params['named']['order_id'])):?>
		<h2><?php echo __l('Payment process');?></h2>
	<?php endif;?>
    <div class="clearfix">
        ...
        <dl class="clearfix guest request-list1">
			...
			<?php endif; ?>
					<dl class="request-list1 view-list1 guest clearfix">
                      ...
                      <dd class="positive-feedback1 js-view-count-property-id js-view-count-property-id-<?php echo $itemDetail['Property']['id']; ?> {'id':'<?php echo $itemDetail['Property']['id']; ?>'}">
                        <?php  echo $this->Html->cInt($itemDetail['Property']['property_view_count']); ?>
                      </dd>
                  </dl>
		           <dl class="ratings-feedback1 clearfix">
                      ...
                      <dd class="positive-feedback1">
                        <?php  echo $this->Html->cInt($itemDetail['Property']['positive_feedback_count']); ?>
                      </dd>
                  </dl>
                   <dl class="ratings-feedback1 clearfix">
                      <dt class="negative-feedback1" title ="<?php echo __l('Negative');?>"><?php echo __l('Negative');?></dt>
                      <dd  class="negative-feedback1">
                            <?php  echo $this->Html->cInt($itemDetail['Property']['property_feedback_count'] - $itemDetail['Property']['positive_feedback_count']); ?>
                      </dd>
                    </dl>
                          ...
                          <?php if(empty($itemDetail['Property']['positive_feedback_count'])): ?>
							...
							<?php else:?>
								 <dd class="success-rate">
										<?php echo $this->Html->image('http://chart.googleapis.com/chart?cht=p&chd=t:'.round($this->Html->cInt($itemDetail['Property']['positive_feedback_count'])).','.(100 - round($this->Html->cInt($itemDetail['Property']['positive_feedback_count']))).'&chs=50x50&chco=00FF00|FF0000&chf=bg,s,FFFFFF00', array('width'=>'50px','height'=>'50px','title' => round($this->Html->cInt($itemDetail['Property']['positive_feedback_count'])).'%')); ?>
								</dd>
							<?php endif; ?>
    		   </dl>
		</div>
		  ...
		  <div class="clearfix edit-info-block">
		  	<div class="rating-block grdi_left">
                 ...
                 <dl class="clearfix request-list grid_2 omega alpha">
				  ...
				  <?php if(isset($this->request->params['named']['type']) && $this->request->params['named']['type']=='related' ): ?>
					  <?php if($this->Auth->user('id')!=$itemDetail['Property']['user_id']): ?>
						  <div class="clearfix">
						 <div class="cancel-block"><?php echo $this->Html->link(__l('Book it!'), array('controller' => 'properties', 'action' => 'view',$itemDetail['Property']['slug'], 'admin' => false), array('title'=>__l('Make an offer'), 'escape' => false)); ?></div></div>
					 <?php endif; ?>
				 <?php endif; ?>
         </div>
         </div>
...
<div class="donations  user-profile-form round-5 pledge-form donations-block clearfix js-submit-target-block"> 
 <?php echo $this->Form->create('Payment', array('action' =>'property_verify_now', 'class' => 'js-submit-target normal clearfix'));
 ...
 <div class="page-information clearfix">
    ...
    <dl class="list payment-list round-5 clearfix"><?php $i = 0; $class = ' class="altrow"';?>
        <dt<?php if ($i % 2 == 0) echo $class;?>><?php echo __l('Verification Fee');?></dt>
			...
			<div class="clearfix">
				<?php if (!empty($gateway_options['paymentGateways'][ConstPaymentGateways::PayPal])): ?>
					<div class="js-paypal-main <?php echo (!empty($this->request->data['Payment']['payment_gateway_id']) && $this->request->data['Payment']['payment_gateway_id'] == ConstPaymentGateways::PayPal) ? "" : 'hide'?> ">
						<div class="js-normal-paypal">
							<div class="pay-paypa-side1 wallet-side1">
				...
				<?php if (!empty($gateway_options['paymentGateways'][ConstPaymentGateways::Wallet])): ?>
					<div class="js-wallet-connection pay-paypa-side1 wallet-side1 <?php echo (!empty($this->request->data['Payment']['payment_gateway_id']) && $this->request->data['Payment']['payment_gateway_id'] == ConstPaymentGateways::Wallet) ? "" : 'hide'?>">
						...
						<div class="clearfix connected-paypal-block">
							<?php echo $this->Form->submit(__l('Pay by Wallet'), array('name' => 'data[Payment][wallet]', 'class' => ' js-update-order-field'));?>
						</div>
					</div>				
				<?php endif; ?>
