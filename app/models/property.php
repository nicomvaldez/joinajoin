...
class Property extends AppModel
{
    ...
    public $hasMany = array(
        ...
        'PropertiesExtraJoinables' => array(
            'className' => 'Properties_extra_joinables',
            'foreignKey' => 'property_id',
            'dependent' => false,
            'conditions' => '',
            'fields' => '',
    ...
    function __construct($id = false, $table = null, $ds = null)
    {
        ...
        $this->validate = array(
            ...
            'state_id' => array(
             ...
             //quito restricciones, ojo con la base de datos despues
/*
            'country_id' => array(
                'rule' => 'notempty',
                'allowEmpty' => false,
	...
	function checkCustomItemPrice($precio, $cantidad=1){
    ...
    function checkCustomMonthPrice($checkin, $checkout, $property_id, $additional_guest,$convert=false)
    {
        ...
        $days = 1;//getCheckinCheckoutDiff($checkin,$checkout);
    ...
    function checkCustomPrice($checkin, $checkout, $property_id, $additional_guest, $convert=false, $type='')
    {
		if ($checkin > $checkout || $checkin < date('Y-m-d')) {
			return 0;
		}
        ...
        if($type == 'join'){
        	$days = 1;//$this->cantDeDias($checkin, $checkout); //getCheckinCheckoutDiff($checkin,$checkout);
		}else{
        	$days = $this->cantDeDias($checkin, $checkout); //getCheckinCheckoutDiff($checkin,$checkout);
		}	
        $no_days = $days;
       ...
       // $additional_guest = (($additional_guest-$property['Property']['additional_guest']) > 0) ? ($additional_guest-$property['Property']['additional_guest']) : 0;
		...
		//$price = ($additional_guest * $property['Property']['price_per_night']) + $custom_daily_price;
        if ($convert) {
			return $this->siteWithCurrencyFormat($price,false);
		} else {
			return $price;
    ...
    function _getCalendarBookingDates($property_id, $month, $year)
    {
        ...
        foreach($property_user_monthly_bookings as $property_user_monthly_booking) {
            $booked[] = array(
               ...
               //'end_date' => getCheckoutDate($property_user_monthly_booking['PropertyUser']['checkout']),
        ...
        foreach($custom_night_monthly_bookings as $custom_night_monthly_booking) {
            $booked[] = array(
                ...
                //'end_date' => getCheckoutDate($custom_night_monthly_booking['CustomPricePerNight']['end_date']),
        ...
        foreach($custom_week_monthly_bookings as $key => $custom_week_monthly_booking) {
            $available_week[$key] = array(
                ...
                //'end_date' => getCheckoutDate($custom_week_monthly_booking['CustomPricePerWeek']['end_date']),
        ...
        //funcion rediseñada para joins PG
            If($property['Property']['join_or_item_value'] == 'join'){
                $property['Property']['price_per_week'] =  $property['Property']['price_per_night'];//($_data['Property']['price_per_night']*7) -10;
            }else{
               $property['Property']['price_per_week'] = ($_data['Property']['price_per_night']*7) -10;
            }
        $booked['weekly_price'] = !empty($property['Property']['price_per_week']) ? $property['Property']['price_per_week'] : ($property['Property']['price_per_night']);
