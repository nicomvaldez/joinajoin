<?php
class Bnbproperty extends AppModel
{
    
    public $name = 'Bnbproperty';
    ...
    function _validateVideoUrl()
    {
        ...
        if (!(!empty($this->data['Bnbproperty']['video_url']) && $this->Embed->parseUrl($this->data['Bnbproperty']['video_url']))) {
            return false;
        }
        return true;
    }
    ...
    function checkCustomWeekPrice($checkin, $checkout, $property_id, $additional_guest, $convert = false)
    {
        ...
        $additional_guest = (($additional_guest-$property['Bnbproperty']['additional_guest']) > 0) ? ($additional_guest-$property['Bnbproperty']['additional_guest']) : 0;
        $additional_guest_price = 0;
        if ($additional_guest > 0) {
            $additional_guest_price = ($additional_guest*$property['Bnbproperty']['additional_guest_price']) *$days;
        }
        $remaining_weeks = $no_weeks-$records;
        $price = 0;
        if ($remaining_weeks > 0) {
            $item_price = !empty($property['Bnbproperty']['price_per_week']) ? $property['Bnbproperty']['price_per_week'] : $property['Bnbproperty']['price_per_night'];
            if (!empty($property['Bnbproperty']['price_per_week'])) {
    ...
    function checkCustomMonthPrice($checkin, $checkout, $property_id, $additional_guest, $convert = false)
    {
        ...
        if ($remaining_months > 0) {
            $item_price = !empty($property['Bnbproperty']['price_per_month']) ? $property['Bnbproperty']['price_per_month'] : $property['Bnbproperty']['price_per_night'];
            if (!empty($property['Bnbproperty']['price_per_month'])) {
    ...
    function checkCustomPrice($checkin, $checkout, $property_id, $additional_guest, $convert = false)
    {
        ...
        $additional_guest = (($additional_guest-$property['Bnbproperty']['additional_guest']) > 0) ? ($additional_guest-$property['Bnbproperty']['additional_guest']) : 0;
        if ($additional_guest > 0) {
            $additional_guest_price = ($additional_guest*$property['Bnbproperty']['additional_guest_price']) *$no_days;
        }
        $remaining_days = $no_days-$found;
        $price = ($remaining_days*$property['Bnbproperty']['price_per_night']) +$custom_daily_price+$additional_guest_price;
        if ($convert) {
            return $this->siteWithCurrencyFormat($price, false);
        } else {
            return $price;
    ...
    function getCalendarData($year, $month, $conditions)
    {
        ...
        if (!empty($sites)) {
            foreach($sites as $site) {
                $unverified[] = $site['Bnbproperty']['id'];
            }
            $conditions['NOT']['Bnbproperty.id'] = $unverified;
        }
        for ($i = 1, $j = 1; $i <= 31; $i++, $j++) {
            ...
            $day[$year][$j] = $this->find('count', array(
                'conditions' => array(
                    $conditions,
                    'and' => array(
                        'Bnbproperty.checkin >= ' => $year . '-' . $month . '-' . $i . ' 00:00:00',
                        'Bnbproperty.checkout <= ' => $year . '-' . $month . '-' . $i . ' 23:59:59',
                        'Bnbproperty.is_active' => 1
                    )
                ) ,
                'recursive' => -1
    ...
    function _updateCityPropertyCount()
    {
        ...
        foreach($cityProperties as $cityProperty) {
            if (!empty($cityProperty['Bnbproperty'])) {
                $this->City->updateAll(array(
                    'City.property_count' => count($cityProperty['Bnbproperty']) ,
                ) , array(
    ...
    function __updatePropertyRequest($request_id, $property_id)
    {
        ...
        if (!$property_request) {
            ...
            $this->updateAll(array(
                'Bnbproperty.request_count' => $request_count,
            ) , array(
                'Bnbproperty.id' => $property_id
            ));
    ...
    function _requestPropertyNotificationMail($request_id, $property_id)
    {
        ...
        $emailFindReplace = array(
            ...
            '##ACTIVATION_URL##' => Router::url(array(
                'controller' => 'properties',
                'action' => 'view',
                $request_property['Bnbproperty']['slug'],
            ) , true) ,
    ...
    function _getCalendarBookingDates($property_id, $month, $year)
    {
        ...
        for ($i = 0; $i < 6; $i++) {
            ...
            $booked['week'][$weekcnt]['price'] = $property['Bnbproperty']['price_per_week'];
        }
        ...
        $booked['property_id'] = $property['Bnbproperty']['id'];
        $booked['weekly_price'] = !empty($property['Bnbproperty']['price_per_week']) ? $property['Bnbproperty']['price_per_week'] : ($property['Bnbproperty']['price_per_night']*7);
        $booked['night_price'] = $property['Bnbproperty']['price_per_night'];
        $booked['monthly'] = !empty($property['Bnbproperty']['price_per_month']) ? $property['Bnbproperty']['price_per_month'] : ($property['Bnbproperty']['price_per_night']*30);
        return $booked;
    }
