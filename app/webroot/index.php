...
function permanentCached($requested = null) {
	$gzip_user_type_id = $gzip_user_id = 0;
	if (!empty($_COOKIE['_gz'])) {
		$_gzip = explode('|', $_COOKIE['_gz']);
		if (count($_gzip) == 3) {
			$hashed_val = md5($_gzip[0] . $_gzip[1] . $_COOKIE['PHPSESSID'] . PERMANENT_CACHE_GZIP_SALT);
			if (substr($hashed_val, 0, 7) == $_gzip[2]) {
				$gzip_user_type_id = $_gzip[0];
				$gzip_user_id = $_gzip[1];	
			}
		}
	}
	$debug_mode = DEBUG;
	if (empty($debug_mode) && PERMANENT_CACHE_CHECK && empty($_COOKIE['_flash']) && !in_array($_SERVER['REQUEST_METHOD'], array('POST', 'PUT', 'DELETE')) && !empty($_SERVER['HTTP_ACCEPT_ENCODING']) && strpos($_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip') !== false) {
		...
		if (!empty($gzip_user_type_id) && $gzip_user_type_id == 1) {
			$cache .= '{' . '_admin' . '_user_' . $gzip_user_id . ',' . '_usertype_' . $gzip_user_type_id . '}';
			$cache_folder = 'user';
		} elseif (!empty($gzip_user_type_id)) {
			$cache .= '{' . '_user_' . $gzip_user_id . ',' . '_usertype_' . $gzip_user_type_id . '}';
			$cache_folder = 'user';
		} else {
		...
		if ($filename = glob(APP_PATH . 'tmp' . DS . 'cache' . DS . 'views' . DS . $cache_folder . DS . $cache . '.gz', GLOB_BRACE)) {
			...
			header('X-cache: hit at ' . date('D, d M Y H:i:s \G\M\T'));
...
function updateViews($table, $main_id) {
	...
	$user_id = isset($gzip_user_id) ? $gzip_user_id : 0;
