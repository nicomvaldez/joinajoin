...
(function($){
  ...
  // is_enable_hashbang_url enabled then force #! url
  ...
  var pjax = $.pjax = function( options ) {
    ...
    if(jQuery.inArray(options.url,urlArr) > -1){
        console.log('= success');
  ...
  {
    if( !$.support.pjax &&  __cfg('is_enable_html5_history_api')) {
	  tmp_pathname = window.location.pathname.replace(__cfg('path_relative'), '/');
      window.location = $.siteurl + '#!' + tmp_pathname + window.location.search;
	}
  }
