...
function updatePrice($checkin, $checkout, $property_id, $guest, $type, no_of_days, guest, per_guest_amount, commisssion_amount) {
    ...
    $.ajax( {
        ...
        success: function(responses) {
            ...
            $('.js-property-no_day-night').html($guest);
...
/* hago calculos de turnos */
//primero pregunto si ya los habia cargado (para que volver a cargar?...)
...
if(!$('#turno1').length) {
        ...
        url: __cfg('path_relative') + 'properties/get_turnos',
        ...
        success: function(responses) {
        	var cont = 1;
//si no trae horarios no toco el html
			if(responses != ''){

	        	data = responses;
	        	// deshabilito el select guests
	        	$('#PropertyUserGuests').attr('disabled','disabled');
	        	
	        	$('#turnos_container').html('<h4>Select Time</h4>');
	        	
	        	$.each(data, function(clave, valor){ 
	        		if(valor != ''){
	        			$('#turnos_container').append(' \
											...
											<div class="properties-middle-block clearfix"> \
												...
												<dt style="width: 200px !important;"> <label><span> <input id="turno'+cont+'" type="radio" name="data[PropertyUser][turno_nro]" data-max="'+ valor["0"]["disponible"] +'"  value="'+valor["p"]["turno_nro"]+'"> '+ valor["p"]["hora"] +' | remaining: '+ valor["0"]["disponible"] +' </span></label> </dt> \
												</dl> \
											</div> \
	...
	$.fn.productCalculation = function() {
		...
		if ($('.js-price-for-product', '#properties-view').is('.js-price-for-product')) {
			...
			if (date1<=date2 &&  $checkin >= curent_date) {
				...
				no_of_days = days_between(date1, date2) + 1;
			...
			if ($checkin <= $checkout && $checkin >= curent_date) {
                ...
                $('#js-checkinout-date').html(date('F d, Y', new Date($('#PropertyUserCheckinYear').val() + '/' + $('#PropertyUserCheckinMonth').val() + '/' + $('#PropertyUserCheckinDay').val())));
			} else {
...
jQuery(document).ready(function($) {
    ...
    $('form#PropertyUserAddForm').delegate('#PropertyUserGuests', 'change', function() {
   ...
   $('div#properties-view').delegate('td.js-week-booking', 'mouseenter', function() {
	...
	$('div#properties-view').delegate('td.js-day-booking', 'click', function() {
		...
		//seteo que selecciono dia
        $this.parents('#PropertyUserAddForm').find('#bookitJoin').attr('data-select','si');
        
        //borro el radio de horas para que se vuelva a generar y me refresque los horarios del dia restantes
        $this.parents('#PropertyUserAddForm').find('#turno1').remove();
