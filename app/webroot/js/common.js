        ...
        if ($(selector, 'body').is(selector)) {
            $(selector).each(function(e) {
                ...
                if ($this.find('select[id$="Hour"]').filter(':first').html()) {
                    ...
                    $this.hide().after(full_label + '<div class="timepicker ' + overlabel_class + '">' + overlabel_time + '<input type="text" class="timepickr" id="caketime' + j + '" title="Select time" readonly="readonly" size="10"/></div>' + error_message);
...
function inserta_horarios(nro_dia){
    ...
    if (horario == 'undefined' || horario == null){
        $('#contenido').append(
            '<div class="campo100 required"><label>'+nombre_dia+'</label><input id="TurnoDetalle'+nro_dia+'" type="hidden" class="HorasTxt" data-dia="'+nro_dia+'" value="" name="data[Properties_hour][hours_texts]['+nro_dia+']"> <input type="button" value="Edit" class="HorasTxt" data-dia="'+nro_dia+'"> \
            <div class="etiquetaHora" id="view_hours'+nro_dia+'">-</div></div> '
        );
    }else{
        $('#contenido').append(
            '<div class="campo100 required"><label>'+nombre_dia+'</label><input id="TurnoDetalle'+nro_dia+'" type="hidden" class="HorasTxt" data-dia="'+nro_dia+'" value="'+horario+'" name="data[Properties_hour][hours_texts]['+nro_dia+']"> <input type="button" value="Edit" class="HorasTxt" data-dia="'+nro_dia+'"> </div>'
        );
    }
    
                    //coloca evento para dia 9
                $(".HorasTxt").click(function() {
        			var $dia = $(this).data('dia');
        			$('.content_hours'+$dia+' a[name=modal]').click();
				});
...
//al enviar el form verifico que se seleccionara un horario de los radios
	$('#PropertyUserAddForm').submit(function(){
		var cantidad = 0;
		cantidad = $(this).find("input[name*='turno']:checked").length; 
		if(cantidad < 1){
			alert('Please select a TIME for this join');
			return false;	
		}
		
		if( $(this).find('#PropertyUserGuests').val() == 0 ){
...
//********************* Turnos restricciones para booking 04/2013 **************************/
// en guest calendar ya hice que el select se pusiera disabled para evitar que se seleccionen cantidades de guest que no tenemos
     $('#turnos_container').delegate("[id*='turno']", 'click', function(){
		//meto los maximos valores para guest de acuerdo al turno seleccionado
			
		$('#PropertyUserGuests').html('');
		
		var max = $(this).data("max");
		$('#PropertyUserGuests').append('<option value="0"></option>');
		for (i=1; i<=max; i++){
			$('#PropertyUserGuests').append('<option value="'+i+'">'+i+'</option>');
		}
		
		$('#PropertyUserGuests').removeAttr('disabled');
...
//******************************* fin tabs ********************************//
    ...
    $('body').delegate('#finish','click', function()
    {
       ...
       if($this.attr('id') == 'PropertyDescription'){ // si es descrition le muestro mensaje
    ...
    $('div.js-submit-target-block').delegate('.js-payment-type', 'click', function() {
        ...
        if ($this.val() == 1) {
            ...
            $('.js-wallet-connection').removeClass('hide');
