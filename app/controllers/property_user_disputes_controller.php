...
class PropertyUserDisputesController extends AppController
{
    ...
    public function admin_index() 
    {
        ...
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'day') {
            $conditions['TO_DAYS(NOW()) - TO_DAYS(PropertyUserDispute.created) <= '] = 0;
            $this->pageTitle.= __l(' -  today');
        }
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'week') {
            $conditions['TO_DAYS(NOW()) - TO_DAYS(PropertyUserDispute.created) <= '] = 7;
            $this->pageTitle.= __l(' -  in this week');
        }
        if (isset($this->request->params['named']['stat']) && $this->request->params['named']['stat'] == 'month') {
            $conditions['TO_DAYS(NOW()) - TO_DAYS(PropertyUserDispute.created) <= '] = 30;
                ...
                'contain' => array(
                    ...
                    'PropertyUser' => array(
                        'fields' => array(
							...
							'PropertyUser.security_deposit_status',
                        ) ,
                        'User' => array(
                            'fields' => array(
                                'User.id',
