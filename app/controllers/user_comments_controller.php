...
class UserCommentsController extends AppController
{
    ...
    public function admin_add() 
    {
        ...
        if (!empty($this->request->data)) {
            $this->UserComment->create();
            if ($this->UserComment->save($this->request->data)) {
                $this->Session->setFlash(sprintf(__l('Recommendation has been added') , $this->request->data['UserComment']['id']) , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index'
                ));
            } else {
                $this->Session->setFlash(sprintf(__l('Recommendation could not be added. Please, try again.') , $this->request->data['UserComment']['id']) , 'default', null, 'error');
            }
        }
        ...
        if (!empty($this->request->data[$this->modelClass])) {
            ...
            if ($actionid && !empty($selectedIds)) {
                if ($actionid == ConstMoreAction::Delete) {
                    ...
                    $this->Session->setFlash(__l('Checked recommendations has been deleted') , 'default', null, 'success');
                }
            }
        }
        if (!$this->RequestHandler->isAjax()) {
